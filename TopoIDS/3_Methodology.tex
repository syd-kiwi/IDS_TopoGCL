...
For a batch of $N$ graphs, let $A=\{a_1,\ldots,a_N\}$ be the embeddings from one augmented view and $B=\{b_1,\ldots,b_N\}$ be the embeddings from the other augmented view. The contrastive loss treats $(a_i,b_i)$ as a positive pair because both embeddings come from the same original graph. It treats $(a_i,b_j)$ for $i\neq j$ as negative pairs because the embeddings come from different graphs. The one-direction InfoNCE loss is:
\begin{equation}
