...
def build_flow_graphs_from_csv(
    input_csv: Path,
    folder_name: str,
    family: str,
    class_id: int,
    ...
    can_use_flow_pipeline = time_col is not None and src_col is not None and dst_col is not None and not missing_edge_cols
    if not can_use_flow_pipeline:
        ...
        missing.extend(missing_edge_cols)
        print(
            f"[INFO] {input_csv} lacks flow graph columns {missing}; "
...
def build_graph_cache(config: ExperimentConfig, label_to_id: Dict[str, int]) -> None:
    ...
    for folder, family in mapped_dataset_folders(config.dataset_root, config.graph_cache_dir):
        ...
        for input_csv in csv_paths:
            graph_count, total_rows_read = build_flow_graphs_from_csv(
                input_csv=input_csv,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset-root", type=Path, default=Path("/home/kiwi-pandas/Documents/IDS_TopoGCL/datasets/CIC_IOT_Dataset2023"))
    parser.add_argument("--dataset", default="CICIoT2023")
    parser.add_argument("--graph-cache-dir", type=Path, default=Path("/home/kiwi-pandas/Documents/IDS_TopoGCL/datasets/CIC_IOT_Dataset2023/Graph_Multiclass"))
    parser.add_argument("--out-json", type=Path, default=Path("/home/kiwi-pandas/Documents/IDS_TopoGCL/results/cic_iot/cic_iot_multiclass_results.json"))
    parser.add_argument("--out-csv", type=Path, default=Path("/home/kiwi-pandas/Documents/IDS_TopoGCL/results/cic_iot/cic_iot_multiclass_results.csv"))
