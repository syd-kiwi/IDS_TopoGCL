...
MODELS = ["svm", "gnn", "topogcl"]
OUTPUT_DIR = "results/limited_labels/OPTC"
...
def main():
    ...
    for ratio in LABEL_RATIOS:
        ...
        val_scores = svm.predict_proba(x_val)[:, 1]
        test_scores = svm.predict_proba(x_test)[:, 1]
        thr = choose_threshold(y_val, val_scores)
        metrics = evaluate(y_test, test_scores, thr)
        res = {"dataset": DATASET, "model": "svm", "label_ratio": ratio,
               "labeled_benign_training_windows": int(nb), "labeled_malicious_training_windows": int(nm),
               "validation_benign_windows": len(b_val), "validation_malicious_windows": len(m_val),
               "test_benign_windows": len(b_test), "test_malicious_windows": len(m_test),
               "threshold_selection_method": THRESHOLD_METHOD, "threshold": float(thr), **metrics}
        ...
        thr = choose_threshold(y_val, val_scores)
        metrics = evaluate(y_test, test_scores, thr)
        res = {"dataset": DATASET, "model": "gnn", "label_ratio": ratio,
               "labeled_benign_training_windows": int(nb), "labeled_malicious_training_windows": int(nm),
               "validation_benign_windows": len(b_val), "validation_malicious_windows": len(m_val),
               "test_benign_windows": len(b_test), "test_malicious_windows": len(m_test),
               "threshold_selection_method": THRESHOLD_METHOD, "threshold": float(thr), **metrics}
        ...
        thr = choose_threshold(y_val, val_scores)
        metrics = evaluate(y_test, test_scores, thr)
        res = {"dataset": DATASET, "model": "topogcl", "label_ratio": ratio,
               "labeled_benign_training_windows": int(nb), "labeled_malicious_training_windows": int(nm),
               "validation_benign_windows": len(b_val), "validation_malicious_windows": len(m_val),
               "test_benign_windows": len(b_test), "test_malicious_windows": len(m_test),
               "threshold_selection_method": THRESHOLD_METHOD, "threshold": float(thr), **metrics,
               "pretraining_epochs": pre_epochs, "downstream_training_epochs": 20,
               "contrastive_temperature": 0.2, "corruption_type": "node_feature_mask", "corruption_rate": 0.2,
    ...
    for metric, name in [("f1", "f1_vs_label_ratio.png"), ("AUROC", "auroc_vs_label_ratio.png"), ("AUPRC", "auprc_vs_label_ratio.png"), ("recall_at_5_percent_fpr", "recall5fpr_vs_label_ratio.png")]:
        plt.figure(figsize=(6, 4))
        for model in MODELS:
            rs = sorted([r for r in summary_rows if r["model"] == model], key=lambda x: x["label_ratio"])
            plt.plot([r["label_ratio"] for r in rs], [r[metric] for r in rs], marker="o", label=model)
        plt.xlabel("Label ratio")
        plt.ylabel(metric)
        plt.title(f"{metric} vs label ratio")
        plt.legend()
        plt.tight_layout()
